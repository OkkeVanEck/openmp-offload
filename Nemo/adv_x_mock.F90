...
contains 
    ...
    SUBROUTINE adv_x_mock_data_simd &
            (jpi, jpj, pdt, put, pcrh, psm, ps0, psx, psxx, psy , psyy, psxy, &
            e1e2t, tmask)
        !!---------------------------------------------------------------------
        !! - Routine: adv_x_mock_data_simd
        !! - Purpose: Parallelized version from the adv_x_mock_data_simd branch
        !! - Source: https://earth.bsc.es/gitlab/amedvede/nemogcm_v40/-/blob/gpu_icedyn_adv_xy_data_simd/src/ICE/icedyn_adv_pra.F90
        !!---------------------------------------------------------------------
        INTEGER                   , INTENT(in   ) ::   jpi, jpj           ! Dimension of the workspace.
        REAL(wp)                  , INTENT(in   ) ::   pdt                ! the time step
        REAL(wp)                  , INTENT(in   ) ::   pcrh               ! call adv_x then adv_y (=1) or the opposite (=0)
    ...
    write (*,*) "IN: adv_x_mock_data_simd"
        ...
        !$omp& map(alloc:zf0,zfx,zfy,zbet,zfm,zfxx,zfyy,zfxy,zalg,zalg1,zalg1q)&
        !$omp& map(to:e1e2t,tmask,put) map(tofrom:psm,ps0,psx,psy,psxx,psyy,psxy)
        DO jl = 1, jcat   ! loop on categories
            !
            ! Limitation of moments.                                           
            DO jj = jjmin, jjmax
                ...
                !$omp parallel do simd private(ztemp,zalf,zbt,zs2new,zalf1,zalf1q,&
                !$omp& zbt1,zpsm,zps0,zpsx,zpsy,zpsxx,zpsyy,zpsxy,rswitch)
                DO ji = fs_2, fs_jpim1 
                    !
                    zpsm  = psm (ji,jj,jl) ! optimization
                    zps0  = ps0 (ji,jj,jl)
...
#ifdef DEBUG_ON
    write (*,*) "OUT: adv_x_mock_data_simd"
    write (*,*) ""
#endif
    END SUBROUTINE adv_x_mock_data_simd


    SUBROUTINE adv_x_mock_data &
            (jpi, jpj, pdt, put, pcrh, psm, ps0, psx, psxx, psy , psyy, psxy, &
            e1e2t, tmask)
        !!---------------------------------------------------------------------
    ...
    write (*,*) "IN: adv_x_mock_data"
        ...
        DO nn = 1, (jcat * (jjmax-jjmin+1)) ! loop on categories and limitation of moments
            ...
            jj = MOD((nn-1), (jjmax-jjmin+1)) + jjmin   
...
#ifdef DEBUG_ON
    write (*,*) "OUT: adv_x_mock_data"
    write (*,*) ""
#endif
    END SUBROUTINE adv_x_mock_data
