! Implements the rc_collapsed version of adv_x_mock based on adv_x_mock.F90.

MODULE Nemo_Adv_X_Rc_Collapsed
    ...
    public :: adv_x_mock_rc_collapsed

contains

   SUBROUTINE adv_x_mock_rc_collapsed &
   (jpi, jpj, ncats, &
   pdt, put, pcrh, psm, ps0, psx, psxx, psy , psyy, psxy, e1e2t, tmask)
   !!---------------------------------------------------------------------
   !! - Routine: adv_x_mock_rc_collapsed
   !! - Purpose: Parallelized version called rc with a manual collapse.
   !!---------------------------------------------------------------------
   ...
   REAL(wp), DIMENSION(jpi,jpj,ncats) ::   zf0 , zfx  , zfy   , zbet   ! 2D workspace
   REAL(wp), DIMENSION(jpi,jpj,ncats) ::   zfm , zfxx , zfyy  , zfxy   !  -      -
   REAL(wp), DIMENSION(jpi,jpj,ncats) ::   zalg, zalg1, zalg1q         !  -      -
   ...
   DO nn = 1, (jcat * (jjmax - jjmin + 1)) ! loop on categories and limitation of moments
      ...
      jj = jjmin + MOD(nn - 1, jjmax - jjmin + 1)
         ...
         DO ji = 1, jpi
            ...
            zbet(ji,jj,jl)  =  MAX( 0._wp, SIGN( 1._wp, put(ji,jj) ) )
            zalf         =  MAX( 0._wp, put(ji,jj) ) * pdt / zpsm
            zalfq        =  zalf * zalf
            zalf1        =  1.0 - zalf
            zalf1q       =  zalf1 * zalf1
            !
            zfm (ji,jj,jl)  =  zalf  *   zpsm 
            zf0 (ji,jj,jl)  =  zalf  * ( zps0  + zalf1 * ( zpsx + (zalf1 - zalf) * zpsxx ) )
            zfx (ji,jj,jl)  =  zalfq * ( zpsx  + 3.0 * zalf1 * zpsxx )
            zfxx(ji,jj,jl)  =  zalf  *   zpsxx * zalfq
            zfy (ji,jj,jl)  =  zalf  * ( zpsy  + zalf1 * zpsxy )
            zfxy(ji,jj,jl)  =  zalfq *   zpsxy
            zfyy(ji,jj,jl)  =  zalf  *   zpsyy

            !                                !  Readjust moments remaining in the box.
            zpsm  =  zpsm  - zfm(ji,jj,jl)
            zps0  =  zps0  - zf0(ji,jj,jl)
            zpsx  =  zalf1q * ( zpsx - 3.0 * zalf * zpsxx )
            zpsxx =  zalf1  * zalf1q * zpsxx
            zpsy  =  zpsy  - zfy (ji,jj,jl)
            zpsyy =  zpsyy - zfyy(ji,jj,jl)
            zpsxy =  zalf1q * zpsxy
            !
         ...
         DO ji = 1, fs_jpim1
            ...
            zalfq         = zalf * zalf
            zalf1         = 1.0 - zalf
            zalg1 (ji,jj,jl) = zalf1
            zalf1q        = zalf1 * zalf1
            zalg1q(ji,jj,jl) = zalf1q
            !
            zfm   (ji,jj,jl) = zfm (ji,jj,jl) + zalf  *    psm (ji+1,jj,jl)
            zf0   (ji,jj,jl) = zf0 (ji,jj,jl) + zalf  * (  ps0 (ji+1,jj,jl) &
               &                                   - zalf1 * ( psx(ji+1,jj,jl) - (zalf1 - zalf ) * psxx(ji+1,jj,jl) ) )
            zfx   (ji,jj,jl) = zfx (ji,jj,jl) + zalfq * (  psx (ji+1,jj,jl) - 3.0 * zalf1 * psxx(ji+1,jj,jl) )
            zfxx  (ji,jj,jl) = zfxx(ji,jj,jl) + zalf  *    psxx(ji+1,jj,jl) * zalfq
            zfy   (ji,jj,jl) = zfy (ji,jj,jl) + zalf  * (  psy (ji+1,jj,jl) - zalf1 * psxy(ji+1,jj,jl) )
            zfxy  (ji,jj,jl) = zfxy(ji,jj,jl) + zalfq *    psxy(ji+1,jj,jl)
            zfyy  (ji,jj,jl) = zfyy(ji,jj,jl) + zalf  *    psyy(ji+1,jj,jl)
         END DO
         !$omp end parallel do simd
         ...
         DO ji = fs_2, fs_jpim1 
            ...
            !                                !  Readjust moments remaining in the box.
            zbt  =       zbet(ji-1,jj,jl)
            zbt1 = 1.0 - zbet(ji-1,jj,jl)
            !
            zpsm  = zbt * zpsm + zbt1 * ( zpsm - zfm(ji-1,jj,jl) )
            zps0  = zbt * zps0 + zbt1 * ( zps0 - zf0(ji-1,jj,jl) )
            zpsx  = zalg1q(ji-1,jj,jl) * ( zpsx + 3.0 * zalg(ji-1,jj,jl) * zpsxx )
            zpsxx = zalg1 (ji-1,jj,jl) * zalg1q(ji-1,jj,jl) * zpsxx
            zpsy  = zbt * zpsy  + zbt1 * ( zpsy  - zfy (ji-1,jj,jl) )
            zpsyy = zbt * zpsyy + zbt1 * ( zpsyy - zfyy(ji-1,jj,jl) )
            zpsxy = zalg1q(ji-1,jj,jl) * zpsxy

            !   Put the temporary moments into appropriate neighboring boxes.    
            !                                !   Flux from i to i+1 IF u GT 0.
            zbt   =       zbet(ji-1,jj,jl)
            zbt1  = 1.0 - zbet(ji-1,jj,jl)
            zpsm  = zbt * ( zpsm + zfm(ji-1,jj,jl) ) + zbt1 * zpsm
            zalf  = zbt * zfm(ji-1,jj,jl) / zpsm
            zalf1 = 1.0 - zalf
            ztemp = zalf * zps0 - zalf1 * zf0(ji-1,jj,jl)
            !
            zps0  =  zbt  * ( zps0 + zf0(ji-1,jj,jl) ) + zbt1 * zps0
            zpsx  =  zbt  * ( zalf * zfx(ji-1,jj,jl) + zalf1 * zpsx + 3.0 * ztemp ) + zbt1 * zpsx
            zpsxx =  zbt  * ( zalf * zalf * zfxx(ji-1,jj,jl) + zalf1 * zalf1 * zpsxx                            &
               &            + 5.0 * ( zalf * zalf1 * ( zpsx  - zfx(ji-1,jj,jl) ) - ( zalf1 - zalf ) * ztemp ) ) &
               &            + zbt1 * zpsxx
            zpsxy =  zbt  * ( zalf * zfxy(ji-1,jj,jl) + zalf1 * zpsxy            &
               &            + 3.0 * (- zalf1*zfy(ji-1,jj,jl)  + zalf * zpsy ) )  &
               &            + zbt1 * zpsxy
            zpsy  =  zbt  * ( zpsy  + zfy (ji-1,jj,jl) ) + zbt1 * zpsy 
            zpsyy =  zbt  * ( zpsyy + zfyy(ji-1,jj,jl) ) + zbt1 * zpsyy

            !                                !  Flux from i+1 to i IF u LT 0.
            zbt   =       zbet(ji,jj,jl)
            zbt1  = 1.0 - zbet(ji,jj,jl)
            zpsm  = zbt * zpsm + zbt1 * ( zpsm + zfm(ji,jj,jl) )
            zalf  = zbt1 * zfm(ji,jj,jl) / zpsm
            zalf1 = 1.0 - zalf
            ztemp = - zalf * zps0 + zalf1 * zf0(ji,jj,jl)
            !
            zps0  = zbt * zps0  + zbt1 * ( zps0 + zf0(ji,jj,jl) )
            zpsx  = zbt * zpsx  + zbt1 * ( zalf * zfx(ji,jj,jl) + zalf1 * zpsx + 3.0 * ztemp )
            zpsxx = zbt * zpsxx + zbt1 * ( zalf * zalf * zfxx(ji,jj,jl) + zalf1 * zalf1 * zpsxx &
               &                         + 5.0 * ( zalf * zalf1 * ( - zpsx + zfx(ji,jj,jl) )    &
               &                         + ( zalf1 - zalf ) * ztemp ) )
            zpsxy = zbt * zpsxy + zbt1 * ( zalf * zfxy(ji,jj,jl) + zalf1 * zpsxy  &
               &                         + 3.0 * ( zalf1 * zfy(ji,jj,jl) - zalf * zpsy ) )
            zpsy  = zbt * zpsy  + zbt1 * ( zpsy  + zfy (ji,jj,jl) )
            zpsyy = zbt * zpsyy + zbt1 * ( zpsyy + zfyy(ji,jj,jl) )
            !
            psm (ji,jj,jl) = zpsm  ! optimization
            ps0 (ji,jj,jl) = zps0 
            psx (ji,jj,jl) = zpsx 
    ...
    END SUBROUTINE adv_x_mock_rc_collapsed

END MODULE Nemo_Adv_X_Rc_Collapsed
