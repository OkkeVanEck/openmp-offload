! Implements the CPU version of adv_x_mock based on adv_x_mock.F90.

MODULE Nemo_Adv_X_CPU
    ...
    public :: adv_x_mock_cpu

contains

    SUBROUTINE adv_x_mock_cpu &
        (jpi, jpj, pdt, put, pcrh, psm, ps0, psx, psxx, psy , psyy, psxy, &
        e1e2t, tmask)
    !!---------------------------------------------------------------------
    !! - Routine: adv_x_mock_cpu
    !! - Purpose: CPU version.
    !!---------------------------------------------------------------------
...
#ifdef DEBUG_ON
    write (*,*) "OUT: adv_x_mock_cpu"
    write (*,*) ""
#endif
    END SUBROUTINE adv_x_mock_cpu

END MODULE Nemo_Adv_X_CPU
